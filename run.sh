# DP
python mnist_dp.py --epochs=1 --batch-size=256

# DDP launch.py
# 1 nodes
python -m torch.distributed.launch --nproc_per_node=4 --nnodes=1 --node_rank=0 --master_addr="10.252.192.48" --master_port=22222 mnist_ddp_launch.py --epochs=1 --batch-size=256
# 2 nodes
# host 执行，要求每台机器 nproc_per_node 相等，不然计算的 world_size 不同
python -m torch.distributed.launch --nproc_per_node=4 --nnodes=2 --node_rank=0 --master_addr="10.252.192.49" --master_port=22222 mnist_ddp_launch.py --epochs=1 --batch-size=256
python -m torch.distributed.launch --nproc_per_node=4 --nnodes=2 --node_rank=1 --master_addr="10.252.192.49" --master_port=22222 mnist_ddp_launch.py --epochs=1 --batch-size=256
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --nnodes=2 --node_rank=0 --master_addr="10.252.192.49" --master_port=22222 mnist_ddp_launch.py --epochs=1 --batch-size=256
CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nproc_per_node=2 --nnodes=2 --node_rank=1 --master_addr="10.252.192.49" --master_port=22222 mnist_ddp_launch.py --epochs=1 --batch-size=256


# DDP mp.spawn
# tcp
# 1 nodes 1m
python mnist_ddp_mp.py --nproc_per_node=4 --nnodes=1 --node_rank=0 --dist-url="tcp://10.252.192.48:22222" --epochs=1 --batch-size=256
# 2 nodes 2m
...
# file
# 1 nodes
python mnist_ddp_mp.py --nproc_per_node=2 --nnodes=1 --node_rank=0 --dist-url="file:///export/nfs/xs/codes/pytorch_operator_example/sharedfile" --epochs=1 --batch-size=256
# 2 nodes
